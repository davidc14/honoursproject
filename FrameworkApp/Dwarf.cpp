...
Dwarf::Dwarf(IDirect3DDevice9* Device)
{
	...
	if(m_Model != NULL)
	{
		...
		if(!LoadModel())
		{
			...
			::MessageBox(0, "Dwarf model failed to load", "Ship error", 0);		
			return;
		}
	}
	else
	{
		...
		::MessageBox(0, "Dwarf pointer failed to instantiate", "Ship error", 0);
	}
...
Dwarf::Dwarf(IDirect3DDevice9* Device, char* filePath, char* fileName)
{
	//Assign memory for the model objects
	m_Model = new XModel(Device);
...
bool Dwarf::LoadModel(char* filePath, char* fileName)
{
	//Attempt to load the model
