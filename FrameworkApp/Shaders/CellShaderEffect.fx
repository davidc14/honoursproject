...
float4 mPixelShader(float2 texCoord : TEXCOORD0, uniform bool applyEdgeDetect,
                                                ...
                                                uniform bool sketchInColor) : COLOR0
{
	...
	if(desatValue > 0.0)
	{
		float intensity = 0.3 * scene.r + 0.59 * scene.g + 0.11 * scene.b;
	   
		scene.r = saturate(intensity * desatValue + scene.r * (1 - desatValue));
		scene.g = saturate(intensity * desatValue + scene.g * (1 - desatValue));
		scene.b = saturate(intensity * desatValue + scene.b * (1 - desatValue));  
	} 
...
technique EdgeDetect
{
    pass P0
    {
        PixelShader = compile ps_2_0 mPixelShader(true, false, false);
    }
}

// Everything off save for desaturation
technique DesaturationOnly
{
    pass P0
    {
        PixelShader = compile ps_2_0 mPixelShader(false, false, false);
    }
}
...
technique EdgeDetectMonoSketch
{
    pass P0
    {
        PixelShader = compile ps_2_0 mPixelShader(true, true, false);
    }
}
...
technique EdgeDetectColorSketch
{
    pass P0
    {
        PixelShader = compile ps_2_0 mPixelShader(true, true, true);
    }
}
...
technique MonoSketch
{
    pass P0
    {
        PixelShader = compile ps_2_0 mPixelShader(false, true, false);
    }
}
...
technique ColorSketch
{
    pass P0
    {
        PixelShader = compile ps_2_0 mPixelShader(false, true, true);
    }
}